...
export default function UjianTahfidzPage() {
  ...
  const handleEditRow = (row: UjianRecord) => {
    const sId = row.santri_id || allSantri.find(s => s.nama_lengkap === row.santri?.nama_lengkap)?.id;
    if (sId) setSelectedSantriId(sId);
    if (row.jenis_ujian) setJenisUjian(row.jenis_ujian as any);
    ...
    if (row.juz) setJuz(row.juz);
    if (row.jumlah_halaman) setJumlahHalaman(row.jumlah_halaman);
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, minWidth: 850 }}>
              <thead>
                <tr style={{ background: "#f8fafc", borderBottom: "2px solid #e2e8f0" }}>
                  {["Tanggal", "Santri", "Jenis Ujian", "Materi", "Nilai Akhir", "Penguji", "Aksi"].map((h, i) => (
