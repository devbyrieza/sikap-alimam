...
export async function syncScheduleFromPDF() {
  try {
    const existingCount = await prisma.jadwalPelajaran.count();
    if (existingCount > 0) {
      return; // Sudah terisi, tidak perlu re-sync otomatis
    }

    console.log("[AutoSyncSchedule] Seed `jadwal_pelajaran` dengan Jadwal Resmi Revisi 2 Agustus 2026...");
    ...
    console.log(`[AutoSyncSchedule] Berhasil memasukkan ${countCreated} jadwal resmi!`);
  } catch (err) {
    console.error("[AutoSyncSchedule] Error:", err);
  }
}
